...
int mixedupness(int* array, unsigned int size){
  // TODO: Implement me!!
  // thoughts: choose the first element of the array as target, then compare it with the rest elements,
  // if the first one is greater than the rest, then increase count.
  // and then choose the second element as the target, and compare as well.
  // the targets will be from index 0 to size - 2 because the last element doesn't 
  // have the rest element to compare, we don't need to care about it.
  // the rest will start from the next element of the target to the last element of the array.
  int count = 0;
  int i, j;
  for (i = 0; i < size - 1; i++) {
    for (j = i + 1; j < size; j++) {
