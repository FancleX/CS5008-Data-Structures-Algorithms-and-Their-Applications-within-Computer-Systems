...
int findMinimum(int* array, int start, int stop){
    ...
    // assume the first one is the min
    // the min value
    int min = array[start];
    // the min value index
    int index = start;
    ...
    for (i = start + 1; i < stop; i++) {
      // if it is not the min, update the value and index to the min one
      if (array[i] < min) {
          min = array[i];
          index = i;
      }
...
void sortIntegers(int* array, unsigned int size){
    ...
    for (i = 0; i < size; i++) {
      // find the min value
      int index = findMinimum(array, i, size);
      // swap the min with current first element, and it will be marked as sorted
