...
// Replicate the given dll and return a copy of the dll
dll_t * replicate(dll_t* l) {
    if (l == NULL) {
        exit(1);
    }
    // create a new dll
    dll_t * newList = create_dll();
    node_t * iterator = l->head;
    while (iterator != NULL) {
        dll_push_back(newList, iterator->data);
...
// Determine if the dll has the value
// return 1 if it has, otherwise 0
int isContain(dll_t* l, void* value) {
    // check null
    if (l == NULL) {
        exit(1);
    }
...
// Get index of a value in the dll
// return the index of the given value
// return -1 if the value is not in the dll
int getIndex(dll_t* l, void* value) {
    if (l == NULL) {
        exit(1);
    }
    int index = 0;
...
void* dll_remove(dll_t* l, int pos){
	...
	if (pos == 0) {
		// call pop_front
		void* result = dll_pop_front(l);
		return result;
	}
	// if remove tail
	if (pos == l->count - 1) {
		// call pop_back
		void* result = dll_pop_back(l);
		return result;
	}
