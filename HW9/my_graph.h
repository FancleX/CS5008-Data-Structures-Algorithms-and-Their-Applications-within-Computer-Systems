...
dll_t* getInNeighbors( graph_t * g, int value ){
    ...
    return !res ? NULL : res->inNeighbors;
}
...
dll_t* getOutNeighbors( graph_t * g, int value ){
    ...
    graph_node_t * res = find_node(g, value);
    return !res ? NULL : res->outNeighbors;
}
...
int contains_edge( graph_t * g, int source, int destination){
    ...
    int result = 0;
    // find the edge in source out
    node_t * iterator = src_out->head;
    while (iterator != NULL) {
        graph_edge_t * edge = iterator->data;
    ...
    // find the edge in destination in
    iterator = des_in->head;
    while (iterator != NULL) {
        graph_edge_t * edge = iterator->data;
        if (edge->from == source && edge->to == destination) {
            result++;
...
graph_edge_t* graph_get_edge(graph_t * g, int source, int destination) {
    ...
    // if the source or destination nodes don't exist
    // contains_edge() will catch this and return 0
    // then this method will return null
    if (contains_edge(g, source, destination) != 1) {
        return NULL;
    }
    // get src out neighbors
    dll_t * src_out = getOutNeighbors(g, source);
    node_t * iterator = src_out->head;
    while (iterator != NULL) {
        graph_edge_t * edge = (graph_edge_t*) iterator->data;
...
int graph_remove_edge(graph_t * g, int source, int destination){
    ...
    // get the graph edge
    graph_edge_t * edge = graph_get_edge(g, source, destination);
    ...
    int src_index = getIndex(src_out, edge);
    // the source index in the des_in dll
    int des_index = getIndex(des_in, edge);
...
int graph_remove_node(graph_t* g, int value){
    ...
    while (iterator != NULL) {
        graph_edge_t * des = (graph_edge_t *) iterator->data;
        graph_remove_edge(g, value, des->to);
        iterator = iterator->next;
    }
    ...
    while (iterator != NULL) {
        graph_edge_t * src = (graph_edge_t *) iterator->data;
        graph_remove_edge(g, src->from, value);
        iterator = iterator->next;
    }
...
int graph_add_edge(graph_t * g, int source, int destination, int cost){
    ...
    dll_t * des_in = getInNeighbors(g, destination);
    // update in and out neighbors
    if (dll_push_back(src_out, edge) != 1) {
        return 0;
    }
    if (dll_push_back(des_in, edge) != 1) {
        return 0;
    }
    // update edges
    g->numEdges++;
