...
queue_t* create_queue(unsigned int _capacity){
	...
	// check if allocate is successful
	if (myQueue == NULL) {
		return NULL;
	}
...
int queue_enqueue(queue_t* q, int item){
	// TODO: Implement me!
	// operate when the queue is not null and full
	if (q != NULL && queue_full(q)==0) {
		// assign the value	
		q->data[q->back] = item;
		// update properites
		q->back++;
		// back out of the boundary of the array,
		// but there are empty spots at the front of the array
...
int queue_dequeue(queue_t *q){
	...
	// store the first item here
	int result = q->data[q->front];
	// move front to the next one
	q->front++;
	// check boundary
