...
void swap(int* a, int* b){
    ...
    // create a temp variable stores the value
    int temp = *a;
    // assign b's value to a
    *a = *b;
    // assign temp's value to b
    *b = temp;
}
...
void sortIntegers(int* array, unsigned int size){
    ...
    for (i = 1; i < size; i++) {
      // current item
      int item = array[i];
      // compare current item with its left side elements
      int j = i - 1;
      // loop when doesn't reach left bound and the left element is greater than current element
      while (j >= 0 && array[j] > item) {
        // move the elements that are greater than the item to the right side by swapping
