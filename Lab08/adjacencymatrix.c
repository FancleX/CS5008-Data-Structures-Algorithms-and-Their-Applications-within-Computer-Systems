...
void generateGraph(int* g){
    ...
    for(i=0; i < ROWS; i++){
        for(j=0; j < COLUMNS; j++){
            if(i==j){
                g[i*COLUMNS+j] = 0; // Why do we have this condition? a node cannot connect to itself
            }else{
...
int nodeInDegree(int* g, int node){
    // TODO
    // check input
    if (g == NULL || sizeof(g)/sizeof(int) == 0 || (node > ROWS || node > COLUMNS || node < 0)) {
        exit(1);
    }
    // get the column of where the node is
    // count "1"s of the col
    int i;
    int count = 0;
    for (i = 0; i < ROWS; i++) {
...
int nodeOutDegree(int* g, int node){
    ...
    // get the row of where the node is
    // count "1"s of the row
    int i;
    int count = 0;
    for (i = 0; i < COLUMNS; i++) {
...
int isConnected(int* g, int node1, int node2){
    // TODO
    // check input
    if (g == NULL || sizeof(g)/sizeof(int) == 0 || (node1 > ROWS || node1 > COLUMNS || node1 < 0) || (node2 > ROWS || node2 > COLUMNS || node2 < 0)) {
        exit(1);
    }
    // check the row of the node 1 to see if the node is connected to node 2
    int i;
    for (i = 0; i < COLUMNS; i++) {
